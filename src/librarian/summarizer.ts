...
const LIBRARIAN_SYSTEM_PROMPT = `# ROLE
...
Task: Update the memory to reflect these changes. Ensure 'Recent Decisions' is updated and 'Active Tech Debt' is pruned if the bug was fixed.`;
  }
  ...
  async summarizeDiff(
    ...
    const prompt = currentMemory
      ...
      : `New Diff:\n${cleaned}\n\nTask: Create or update PROJECT_MEMORY.md based on this diff.`;

    // If total is small enough, process directly
    if (totalTokens < maxTokens * 0.7) {
      return await generateSummary(prompt, LIBRARIAN_SYSTEM_PROMPT, {
        thinkingLevel,
        maxTokens,
      });
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const summary = await generateSummary(chunkPrompt, LIBRARIAN_SYSTEM_PROMPT, {
        thinkingLevel,
        maxTokens,
      });
...
Task: Create a single, unified PROJECT_MEMORY.md that combines all the information from these summaries.`;
      ...
      return await generateSummary(aggregatedPrompt, LIBRARIAN_SYSTEM_PROMPT, {
...
Create a complete PROJECT_MEMORY.md with all required sections.`;
